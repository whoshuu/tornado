...
def file_log(message, extra="global"):
    with open("/tmp/debug.log", "a") as f:
        f.write("TOR/handlers.py " + extra + " " + message + "\n")
...
class WebSocketProtocol(abc.ABC):
    ...
    def __init__(self, handler: "_WebSocketDelegate") -> None:
        file_log(f"__init__ {handler.request.path}", self.__class__.__name__)
...
class _PerMessageDeflateCompressor(object):
    ...
    def _create_compressor(self) -> "_Compressor":
        file_log("_create_compressor", self.__class__.__name__)
        return zlib.compressobj(
            self._compression_level, zlib.DEFLATED, -self._max_wbits, self._mem_level
        )

    def compress(self, data: bytes) -> bytes:
        file_log("compress", self.__class__.__name__)
...
class _PerMessageDeflateDecompressor(object):
    ...
    def _create_decompressor(self) -> "_Decompressor":
        file_log("_create_decompressor", self.__class__.__name__)
        return zlib.decompressobj(-self._max_wbits)

    def decompress(self, data: bytes) -> bytes:
        file_log("decompress", self.__class__.__name__)
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    def _handle_websocket_headers(self, handler: WebSocketHandler) -> None:
        file_log(f"_handle_websocket_headers {self.handler.request.path}", self.__class__.__name__)
    ...
    def compute_accept_value(key: Union[str, bytes]) -> str:
        file_log(f"compute_accept_value {self.handler.request.path}")
    ...
    def _challenge_response(self, handler: WebSocketHandler) -> str:
        file_log(f"_challenge_response {self.handler.request.path}", self.__class__.__name__)
    ...
    async def _accept_connection(self, handler: WebSocketHandler) -> None:
        file_log(f"_accept_connection {self.handler.request.path}", self.__class__.__name__)
    ...
    def _parse_extensions_header(
        ...
        file_log(f"_parse_extensions_header {self.handler.request.path}", self.__class__.__name__)
    ...
    def _get_compressor_options(
        ...
        file_log(f"_get_compressor_options {self.handler.request.path}", self.__class__.__name__)
    ...
    def _create_compressors(
        ...
        file_log(f"_create_compressors {self.handler.request.path}", self.__class__.__name__)
    ...
    def _write_frame(
        ...
        file_log(f"_write_frame {self.handler.request.path}", self.__class__.__name__)
        data_len = len(data)
        if opcode & 0x8:
    ...
    def write_message(
        ...
        file_log(f"write_message  {self.handler.request.path}WebsocketProtocol13", self.__class__.__name__)
        """Sends the given message to the client of this Web Socket."""
        if binary:
            opcode = 0x2
        else:
    ...
    async def _read_bytes(self, n: int) -> bytes:
        file_log(f"_read_bytes {self.handler.request.path}", self.__class__.__name__)
        data = await self.stream.read_bytes(n)
        self._wire_bytes_in += n
        return data

    async def _receive_frame(self) -> None:
        file_log(f"_receive_frame {self.handler.request.path}", self.__class__.__name__)
    ...
    def _handle_message(self, opcode: int, data: bytes) -> "Optional[Future[None]]":
        file_log(f"_handle_message {self.handler.request.path}", self.__class__.__name__)
    ...
    @property
    def ping_interval(self) -> Optional[float]:
        file_log(f"ping_interval {self.handler.request.path}", self.__class__.__name__)
    ...
    @property
    def ping_timeout(self) -> Optional[float]:
        file_log(f"ping_timeout {self.handler.request.path}", self.__class__.__name__)
    ...
    def periodic_ping(self) -> None:
        file_log(f"periodic_ping {self.handler.request.path}", self.__class__.__name__)
...
class WebSocketClientConnection(simple_httpclient._HTTPConnection):
    ...
    def close(self, code: Optional[int] = None, reason: Optional[str] = None) -> None:
        file_log(f"close {self.path}", self.__class__.__name__)
        """Closes the websocket connection.
    ...
    def on_connection_close(self) -> None:
        file_log(f"on_connection_close {self.path}", self.__class__.__name__)
    ...
    def on_ws_connection_close(
        ...
        file_log(f"on_ws_connection_close {self.path}", self.__class__.__name__)
        self.close_code = close_code
        self.close_reason = close_reason
        self.on_connection_close()

    def _on_http_response(self, response: httpclient.HTTPResponse) -> None:
        file_log(f"_on_http_response {self.path}", self.__class__.__name__)
        if not self.connect_future.done():
            if response.error:
    ...
    async def headers_received(
        ...
        file_log(f"headers_received {self.path}", self.__class__.__name__)
    ...
    def write_message(
        ...
        file_log(f"write_message {self.path}", self.__class__.__name__)
    ...
    def read_message(
        ...
        file_log(f"read_message {self.path}", self.__class__.__name__)
    ...
    def on_message(self, message: Union[str, bytes]) -> Optional[Awaitable[None]]:
        file_log(f"on_message {self.path}", self.__class__.__name__)
        return self._on_message(message)

    def _on_message(
        self, message: Union[None, str, bytes]
    ) -> Optional[Awaitable[None]]:
        file_log(f"_on_message {self.path}", self.__class__.__name__)
    ...
    def ping(self, data: bytes = b"") -> None:
        file_log(f"ping {self.path}", self.__class__.__name__)
        """Send ping frame to the remote end.
    ...
    def on_pong(self, data: bytes) -> None:
        file_log(f"on_pong {self.path}", self.__class__.__name__)
        pass

    def on_ping(self, data: bytes) -> None:
        file_log(f"on_ping {self.path}", self.__class__.__name__)
        pass

    def get_websocket_protocol(self) -> WebSocketProtocol:
        file_log(f"get_websocket_protocol {self.path}", self.__class__.__name__)
        return WebSocketProtocol13(self, mask_outgoing=True, params=self.params)

    @property
    def selected_subprotocol(self) -> Optional[str]:
        file_log(f"selected_subprotocol {self.path}", self.__class__.__name__)
    ...
    def log_exception(
        ...
        file_log(f"log_exception {self.path}", self.__class__.__name__)
